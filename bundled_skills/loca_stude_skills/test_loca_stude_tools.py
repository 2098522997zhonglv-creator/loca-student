...
MODULE_PATH = Path(__file__).with_name("loca_stude_tools.py")
MODULE_SPEC = importlib.util.spec_from_file_location("loca_stude_tools_under_test", MODULE_PATH)
loca_stude_tools = importlib.util.module_from_spec(MODULE_SPEC)
...
MODULE_SPEC.loader.exec_module(loca_stude_tools)
...
class LocaStudeToolsScreenshotResolutionTests(unittest.TestCase):
    def _write_file(self, file_path: str):
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, "wb") as f:
            f.write(b"fake-png")

    @patch.object(loca_stude_tools.requests, "post", return_value=_DummyResponse())
    def test_upload_screenshot_falls_back_to_tmp_screenshots(self, mock_post):
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            with patch.dict(
                ...
                result = loca_stude_tools.upload_screenshot(3, 163, "step1.png", "登录截图")
    ...
    def test_upload_screenshots_falls_back_by_basename_for_missing_screen_dir_path(self, mock_post):
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            with patch.dict(
                ...
                result = loca_stude_tools.upload_screenshots(
                    3,
                    163,
    ...
    def test_upload_screenshot_missing_error_lists_searched_dirs(self):
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            with patch.dict(
                ...
                result = loca_stude_tools.upload_screenshot(3, 163, "missing.png", "缺失截图")
...
class LocaStudeToolsAddModuleTests(unittest.TestCase):
    @patch.object(loca_stude_tools.requests, "post")
    def test_add_module_success(self, mock_post):
        ...
        result = loca_stude_tools.add_module(project_id=1, name="新模块", parent_id=10)
        self.assertEqual(result["message"], "保存成功")
        self.assertEqual(result["module"]["id"], 42)
        self.assertEqual(result["module"]["name"], "新模块")

        mock_post.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/testcase-modules/",
            headers=loca_stude_tools.HEADERS,
    ...
    def test_add_module_failure(self, mock_post):
        ...
        result = loca_stude_tools.add_module(project_id=1, name="新模块")
        self.assertEqual(result["message"], "保存失败")
        self.assertEqual(result["response"]["code"], 400)


class LocaStudeToolsFileManagementTests(unittest.TestCase):
    @patch.object(loca_stude_tools.requests, "get", return_value=_JsonResponse({"count": 1, "results": [{"id": 8}]}))
    def test_list_files_passes_filters(self, mock_get):
        result = loca_stude_tools.list_files(
            project_id=1,
            page=2,
            page_size=50,
            search="需求",
        ...
        mock_get.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/files/",
            headers=loca_stude_tools.HEADERS,
            params={
                "page": 2,
                "page_size": 50,
    ...
    @patch.object(loca_stude_tools.requests, "post", return_value=_JsonResponse({"file_id": 8, "name": "需求.txt"}, 201))
    def test_upload_file_uses_multipart_files_field(self, mock_post):
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            result = loca_stude_tools.upload_files(1, file_path)

        self.assertEqual(result["file_id"], 8)
        mock_post.assert_called_once()
        args, kwargs = mock_post.call_args
        self.assertEqual(args[0], f"{loca_stude_tools.BASE_URL}/api/projects/1/files/")
        self.assertEqual(kwargs["headers"], loca_stude_tools.HEADERS)
    ...
    @patch.object(loca_stude_tools.requests, "post", return_value=_JsonResponse({"valid": True, "files": []}))
    def test_validate_files_parses_csv_ids(self, mock_post):
        result = loca_stude_tools.validate_files(1, "8, 9,8")

        self.assertEqual(result, {"valid": True, "files": []})
        mock_post.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/files/validate/",
            headers=loca_stude_tools.HEADERS,
            json={"file_ids": [8, 9]},
        )

    @patch.object(
        loca_stude_tools.requests,
        "post",
    ...
    def test_update_file_settings_parses_bool_values(self, mock_post):
        result = loca_stude_tools.update_file_settings(1, "true", "false")

        self.assertTrue(result["auto_delete_on_unbind"])
        self.assertFalse(result["auto_delete_zero_refs"])
        mock_post.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/files/settings/",
            headers=loca_stude_tools.HEADERS,
            json={"auto_delete_on_unbind": True, "auto_delete_zero_refs": False},
        )

    @patch.object(loca_stude_tools.requests, "delete")
    def test_delete_file_handles_204(self, mock_delete):
        mock_delete.return_value = _JsonResponse({}, 204)

        result = loca_stude_tools.delete_file(1, 8)

        self.assertTrue(result["success"])
        mock_delete.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/files/8/",
            headers=loca_stude_tools.HEADERS,
        )

    @patch.object(loca_stude_tools.requests, "get")
    def test_download_file_saves_response_content(self, mock_get):
        ...
        with tempfile.TemporaryDirectory() as temp_root:
            result = loca_stude_tools.download_file(1, 8, output_dir=temp_root)
        ...
        mock_get.assert_called_once_with(
            f"{loca_stude_tools.BASE_URL}/api/projects/1/files/8/download/",
            headers=loca_stude_tools.HEADERS,
            stream=True,
        )
