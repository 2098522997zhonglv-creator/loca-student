...
3. 如果页面需要登录，先问用户要 Cookie 或 Token 并用 `--header` 重试；遇到验证码、必须运行 JavaScript，或带上凭证后仍只抓到空壳页面时，改用浏览器 skill 读取页面状态。
...
# 需要登录的页面：用 --header 带上凭证，可重复传多个
python scripts/read_url.py "https://example.com/private" --header "Cookie: session=abc123"
python scripts/read_url.py "https://example.com/private" --header "Authorization: Bearer abc123"
```

凭证会出现在命令和运行日志里，只在用户主动提供时使用，不要自行猜测或长期保存。
